...
`MultiDataSet`s should be created prior to adding any object using our constructor:
...
Subsetting of `MultiDataSet`s can be done by samples, by tables or using a `GenomicRanges`. In order to illustrate these operations, we will use the `ExpressionSet` and the snps of `r Rpackage("MEALData")`.  These sets contain gene expression and snps data from common samples. The `ExpressionSet` will also be added to another slot but setting GRanges = NA:
